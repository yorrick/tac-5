...
Generate a random, interesting natural language query that explores this data. The query should:
- Be maximum TWO sentences
- Be a question that demonstrates different types of SQL operations (filtering, aggregation, sorting, joins if multiple tables)
- Be contextually relevant to the available tables and columns
- Be diverse and interesting (mix simple queries with more complex ones)
- Not mention SQL or technical terms - phrase it naturally as a user would ask
- Be specific and executable

Examples of good queries:
- "Show me the top 5 customers by total spending."
- "What's the average age of users who signed up in the last month?"
- "List all products that are out of stock, sorted by name."
...
Generate ONE random query:"""
        ...
        response = client.chat.completions.create(
            model="gpt-4.1-mini",
            messages=[
                {"role": "system", "content": "You are a helpful assistant that generates natural language queries for data exploration."},
                {"role": "user", "content": prompt}
            ],
...
def generate_random_query(schema_info: Dict[str, Any]) -> str:
    ...
    if openai_key:
        ...
        raise Exception("No LLM API keys available. Set OPENAI_API_KEY or ANTHROPIC_API_KEY environment variable.")